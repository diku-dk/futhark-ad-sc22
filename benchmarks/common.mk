...
%.json: %.py
	$(PYTHON) -c 'import $(basename  $<); $(basename $<).benchmarks(runs=$(RUNS), output="$(basename $<).json")'

%.json: %.fut
	$(FUTHARK) bench $< $(FUTHARK_TUNING) $(FUTHARK_BENCH_OPTIONS) -r $(RUNS) --backend=$(FUTHARK_BACKEND) --json $@
ifeq ($(NAME),$(basename $<))
	$(PYTHON) -c 'import benchmark; benchmark.process_futhark("$(basename $<).json", "$<")'
else
	TYPE=$(patsubst *_%, %, $<)
	$(PYTHON) -c 'import benchmark; benchmark.process_futhark("$(basename $<).json", "$<", "$(TYPE)")'
endif

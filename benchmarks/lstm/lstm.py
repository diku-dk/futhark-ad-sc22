...
def benchmarks(parameters = parameters, runs=10, validate=False, output="lstm_py.json", data_dir="data", prec="f32"):
  ...
  for params in parameters:
    ...
    times[filename] = { 'naive' : naiveLSTM.report(),
...
class NaiveLSTM(nn.Module, Benchmark):
  ...
  def prepare(self):
      ...
      self.W_i, self.W_f, self.W_j, self.W_o = \
           tuple(nn.Parameter(torch.transpose(t, 0, 1)) for t in torch.chunk(self.tensors['weight_ih_l0'], 4))

      self.b_ii, self.b_if, self.b_ij, self.b_io = \
           tuple(nn.Parameter(t) for t in torch.chunk(self.tensors['bias_ih_l0'], 4))

      self.b_hi, self.b_hf, self.b_hj, self.b_ho = \
           tuple(nn.Parameter(t) for t in torch.chunk(self.tensors['bias_hh_l0'], 4))

      self.U_i, self.U_f, self.U_j, self.U_o = \
           tuple(nn.Parameter(torch.transpose(t, 0, 1)) for t in torch.chunk(self.tensors['weight_hh_l0'], 4))
  ...
  def calculate_jacobian(self):
    ...
    self.jacobian = {  'weight_ih_l0': torch.concat([torch.transpose(g, 0, 1) for g in [d['W_i'], d['W_f'], d['W_j'], d['W_o']]])
...
class RNNLSTM(nn.Module, Benchmark):
  ...
  def forward(self, input_,):
    ...
    self.objective = torch.reshape(self.linear(torch.cat([t for t in outputs])), (self.n, self.bs, self.d))
  ...
  def calculate_jacobian(self):
    ...
    self.jacobian = {n: p.grad for n, p in chain(self.lstm.named_parameters(), self.linear.named_parameters())}
